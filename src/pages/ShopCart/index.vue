  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart,index) in cartInfoList" :key="cart.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cart.isChecked" @change="UpdCheckedById(cart,$event)">
          </li>
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="delCartById(cart.skuId)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChecked" @change="updateAllCartChecked($event)">
        <span>全选</span>
      </div>
      <div class="option">
      ...
      <div class="money-box">
        ...
        <div class="sumprice">
          <em>总价（不含运费） ：</em>
          <i class="summoney">{{ cartPriceSum }}</i>
        </div>
        <div class="sumbtn">
  ...
  export default {
    ...
    computed:{
      ...
      isAllChecked(){
        ...
        // return this.cartInfoList.filter()
      }
    },  
    mounted(){
      this.getData();
    ...
    methods:{
      ...
      async handler(type, disNum, cart){
        ...
        //skuId,skuNum
        try {
          let result = await this.$store.dispatch('addOrUpdateCart', {skuId:cart.skuId,skuNum:disNum})
          this.getData()
        } catch (error) {
          alert('添加或修改购物车失败')
        }
      ...
      UpdCheckedById(cart, event){
          // console.log('商品选中状态',event.target.checked)

          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="index">
                ...
                <dd changepirce="0" 
                    :class="{active:valueObj.isChecked==1}" 
                    v-for="(valueObj,index) in spuSaleAttr.spuSaleAttrValueList" 
                    :key="index"
                    @click="changeActive(valueObj,spuSaleAttr.spuSaleAttrValueList)">
                    {{ valueObj.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <!-- 购物车操作 -->
  ...
  export default {
    ...
    methods:{
      ...
      changeActive(attrValue,attrValueList){
        ...
        //排他操作
        //底下的代码:修改数组里面的对象【响应式数据】,数据变化视图跟这变化！！！
      ...
      async addOrUpdateCart(){
        try {
          let result = await this.$store.dispatch("addOrUpdateCart", {
            skuId:this.skuInfo.id,
            skuNum:this.skuNum,
          })
          this.$router.push({
            path:'/addcartsuccess',
            //第一种 只建议传简单的参数，传对象，数组这种参数时会从$route看接收到的参数变成了"[object Object]"
            //这种方法可以传参数但是对象当成query参数传过去的时候,对象参数就变成一个这样的值"[object Object]"
            // query: {skuInfo:this.skuInfo,skuNum: this.skuNum}})
            //第二种 会话存储， 非持久化，浏览器一关闭数据消失
            //      本地存储   持久化   5M 如果5M存满了会怎么样，因为我选择哪个商品到购物车，添加成功后显示的肯定是我
            //刚刚选择，因此只需要会话存储就能满足，如果用本地存储，用户可能买很多商品，这些就会造成存储了太多不必要的东西
